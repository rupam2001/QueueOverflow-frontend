...
export default function AuthContextProvider(props) {
  ...
  const [AuthRespObj, setAuthRespObj] = useState<AuthResp>({
    success: false,
    profile_pic: "",
    name: "name",
  });
  ...
  const AutoAuthFromTokenAsync = async (): Promise<any> => {
    try {
      ...
      //api call with token
      const { success, profile_pic, name } = await fetch(
        ENDPOINT + AutoAuthRoute,
        {
          method: "POST",
  ...
  const AuthenticateAsync = async (tokenId: string): Promise<AuthResp> => {
    try {
      ...
      const { token, success, profile_pic, name } = await fetch(
        ENDPOINT + AuthRoute,
        {
          method: "POST",
  ...
  const AuthenticateAsync2 = async (
    email: string,
    password: string
  ): Promise<AuthResp> => {
    try {
  ...
  const LogoutAsync = async (): Promise<boolean> => {
    try {
      ...
      const { success } = await fetch(ENDPOINT + LogoutRoute, {
        method: "DELETE",
        body: JSON.stringify({ token: token }),
        headers: { "Content-Type": "application/json" },
      }).then((resp) => resp.json());

      if (success) removeCookieToken();
      else return false;
