...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fn = (e) => {
      if (
        e.target.id !== "myDropdown" &&
        !e.target.classList.contains("dd-item") &&
        e.target.id !== "ignore"
      ) {
        // alert("i")
  ...
  useEffect(() => {
    (async () => {
      if (!authContext.isLogin) await authContext.AutoAuthFromTokenAsync();
  ...
  const handleLogout = () => {
    ...
    authContext.LogoutAsync().then((success) => {
      if (success) {
        setAuthInProgress(false);
      } else {
  ...
  const onClickLogin = async () => {
    const email = (document.getElementById("signin_email") as HTMLInputElement)
      .value;
    const password = (
      document.getElementById("signin_pass") as HTMLInputElement
    ).value;
    const resp = await authContext.AuthenticateAsync2(email, password);
    const { success } = resp;
    if (success) {
      var modal = document.getElementById("myModal");
    ...
    <div className="dropdown">
      ...
      <div id="myModal" className="modal">
        <div className="modal-content" style={{}}>
          ...
          >
            <input type="text" placeholder="email" id="signin_email" />
            <input type="password" placeholder="password" id="signin_pass" />
