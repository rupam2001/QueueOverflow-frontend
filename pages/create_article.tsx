...
export default function CreateArticle() {
    ...
    const saveAsDraftLocaly = () => {
        ...
        if (currentDraft) {
            let updated = { name: currentDraft.name, coverphoto: coverPhotoUrl, title: title, body: markdownText, time: new Date().toString() }
            updateDraft(updated)
            // notify("Saved!")
    ...
    useEffect(() => {
        ...
        const { title, body, coverphoto } = currentDraft
        setTitle(title)
        setMarkdownText(body)
        setCOverPhotoUrl(coverphoto)
    }, [currentDraft])
    ...
    const handleDraftModalButton = () => {
        ...
        const draft = { name: draftInputRef.current.value, coverphoto: coverPhotoUrl, title: title, body: markdownText, time: new Date().toString() }
